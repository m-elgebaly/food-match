...
import type { GuestReaction } from '@/lib/types'
...
export default function AuthForm({ defaultTab = 'signin', redirectTo = '/explore' }: AuthFormProps) {
  ...
  async function migrateGuestReactions(userId: string) {
    try {
      const raw = localStorage.getItem('guestReactions')
      if (!raw) return
      const reactions: GuestReaction[] = JSON.parse(raw)
      ...
      localStorage.removeItem('guestReactions')
    } catch {}
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (tab === 'signin') {
      ...
      if (error) {
        ...
        await migrateGuestReactions(data.user.id)
